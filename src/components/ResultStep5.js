...
const ResultStep5 = ({ data }) => {
  const symptomList = data.reaction2[1].symptom;
  const goodHabitList = data.reaction2[3].goodHabit;
  const badHabitList = data.reaction2[4].badHabit;

  return (
    <>
      <HeaderBarImage src={HeaderBar} />
      <SubTitle>부작용. {data.reaction2[0].label}</SubTitle>
      <List>
      ...
      <Description>{data.reaction2[2].explain}</Description>
      <DataList title="이렇게 하면 좋아요" icon={ThumbUpIcon} data={goodHabitList} />
      <DataList title="이렇게 하면 안돼요" icon={ThumbDownIcon} data={badHabitList} />
      <ButtonContainer>
